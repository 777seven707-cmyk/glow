...
.mcp.json                  MCP-серверы проекта (Perplexity) для ИИ-клиентов
.gitignore                 секреты и локальные настройки мимо репозитория
```
...
К проекту подключён MCP-сервер Perplexity — он даёт ИИ-клиенту (Claude Code, Cursor,
VS Code, Claude Desktop) веб-поиск и разбор источников прямо в диалоге. На сам сайт
это не влияет: ни одного запроса к Perplexity из `index.html` или `assets/js/` не идёт,
сервер нужен только во время разработки.

В репозитории лежит `.mcp.json` с удалённым сервером — секретов в нём нет:
...
Удалённый сервер рекомендован: ставить нечего, обновляется сам, вход — через
обычный аккаунт Perplexity по OAuth, ключ в репозиторий не попадает.

**Claude Code.** Конфигурация подхватывается из `.mcp.json` при открытии проекта —
клиент один раз спросит разрешение на сервер из репозитория. Дальше `/mcp` → войти
в `perplexity`. Без файла сервер добавляется командой:

```bash
claude mcp add --transport http perplexity https://api.perplexity.ai/mcp
```

**Cursor** — в `~/.cursor/mcp.json` блок `mcpServers` с полем `url`, затем
Settings → Tools & MCP → Connect рядом с `perplexity`.

**VS Code** — в `.vscode/mcp.json` блок `servers`, где у `perplexity` стоят
`"type": "http"` и тот же `url`; браузер для входа откроется при первом запуске.

При входе выбирается организация API, на которую пойдёт оплата — нужны права
администратора в организации, способной оплачивать использование.

### Вместо входа — ключ API

Клиентам без OAuth ключ передаётся заголовком:

```
Authorization: Bearer YOUR_API_KEY
```

Ключ берётся в консоли Perplexity (`console.perplexity.ai` → project → keys) и
хранится **только локально** — в глобальном конфиге клиента или в `.mcp.local.json`;
оба варианта уже закрыты `.gitignore`. Коммитить ключ нельзя: репозиторий публичный.

### Локальный сервер

Если клиент умеет только stdio или нужна фиксированная версия — тот же сервер
...
Ключ здесь обязателен — OAuth у локального варианта нет.

### Инструменты

| Инструмент | Для чего |
|------------|----------|
| `perplexity_search` | веб-поиск: ранжированные результаты со ссылками и выдержками |
| `perplexity_ask` | короткий ответ с учётом веба (пресет `fast`) |
| `perplexity_research` | развёрнутый разбор темы с цитатами (пресет `high`) |
| `perplexity_reason` | пошаговые рассуждения и разбор задач (пресет `medium`) |

Вызовы тарифицируются по обычным ценам API на тот ключ или организацию, с которыми
выполнено подключение; действуют лимиты этой организации.
