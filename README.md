...
**Форма заявки ничего никуда не отправляет сама.** Она открывает почтовую
программу посетителя с готовым письмом (`mailto:`). Это работает без сервера,
но часть посетителей до отправки не дойдёт: на телефоне почтовая программа
открывается не всегда, а те, кто пользуется почтой в браузере, вообще ничего
не увидят. Поэтому рядом всегда есть Telegram — он надёжнее.

Чтобы заявки приходили гарантированно, форму нужно подключить к сервису
(Formspree, Getform, Google Forms) или к своему серверу — это отдельная
задача, сейчас её нет.
