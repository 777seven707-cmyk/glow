...
Фон секций после первого экрана — два встречных пучка по 30 кривых, по которым
...
Нагрузка удерживается тремя вещами: разметка создаётся лениво, только когда
секция впервые показалась на экране; вне экрана анимация ставится на паузу
через `animation-play-state`; при `prefers-reduced-motion` линии рисуются
статично. Разброс скоростей задан формулой один раз при создании, а не
случайным числом на каждом кадре — иначе любой пересчёт перезапускал бы
все анимации.
