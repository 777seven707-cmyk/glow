...
## Страницы про Varmilo Sakura

`sakura.html` — главная: единый фон с горой, солнцем и падающими лепестками,
крупная типографика, объяснение «что это за сайт» в первом экране и катана,
которая на настоящей 3D-модели (three.js) пролетает через заголовок и
«разрезает» его.

`keyboard-3d.html` — вторая страница: превью на фоне горной гряды, под ним
интерактивная 3D-модель VA87M — раскладка ANSI на 87 клавиш, шаг 19,05 мм,
корпус 356 × 134 × 33 мм, вращение мышкой, зум колесом, четыре вида.

Скрипты: `assets/js/sakura.js` (фон, параллакс, лепестки, появление блоков),
`assets/js/katana3d.js` (модель катаны и взмах), `assets/js/keyboard3d.js`
(модель клавиатуры). three.js r149 лежит локально в `assets/js/vendor/`.
