...
`sakura.html` — обзор клавиатуры Varmilo Sakura VA87M в стиле постера:
японская SVG-сцена, живая катана, CSS-3D-клавиатура, которую крутят мышкой.
`keyboard-3d.html` — отдельная страница с трёхмерной моделью на three.js:
геометрия строится из раскладки ANSI (87 клавиш, шаг 19,05 мм) и габаритов
корпуса 356 × 134 × 33 мм, свет и тени настоящие, вращение и зум — мышкой,
пальцем и стрелками.

three.js лежит локально в `assets/js/vendor/three.min.js` (r149, лицензия
рядом) — CDN не нужен, страница работает офлайн.
